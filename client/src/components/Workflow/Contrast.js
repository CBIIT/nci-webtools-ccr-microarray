...
class Contrast extends Component {
  ...
  render() {
    ...
    let queueBlock = (
      <div className="block ">
        <div id="checkbox_queue">
          {' '}
          <Checkbox checked={this.props.data.useQueue} onChange={this.handleSwitchChange}>
            Submit this job to a Queue
          </Checkbox>
        </div>
        <div className="queueMessage" style={{ paddingLeft: '23px' }}>
          (Jobs currently enqueued: {this.props.data.numberOfTasksInQueue})
        </div>
        ...
        <Input
          disabled={!this.props.data.useQueue}
          id="email"
        ...
        <span className="queueMessage">
          Note: if sending to queue, when computation is completed, a notification will be sent to
          the e-mail entered above.
        </span>
        <div>
    ...
    let button = '';
    if (
      this.props.data.group_1 != '-1' &&
      this.props.data.group_2 != '-1' &&
      this.props.data.group_1 != this.props.data.group_2
    ) {
      button = (
        <div style={{ padding: '8px 5px 10px 5px', margin: '10px' }}>
          <Button
            ...
            onClick={this.props.runContrast}
          >
    ...
    let group_1_content = (
      <select
        ...
        value={this.props.data.group_1}
        style={{ width: '100%' }}
        onChange={this.props.handleGroup1Select}
      >
    ...
    let group_2_content = (
      <select
        id="select-group-2"
        className="ant-select-selection ant-select-selection--single"
        value={this.props.data.group_2}
        style={{ width: '100%' }}
        onChange={this.props.handleGroup2Select}
      >
        <option value="-1">---select Group---</option>
        {options}
      </select>
    ...
    let normalization_option = (
      <select
        ...
        style={{ width: '100%' }}
        value={this.props.data.normal}
        onChange={this.props.handleNormalSelect}
      >
