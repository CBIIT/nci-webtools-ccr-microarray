...
RUN set -e; \
    Rscript -e "install.packages('webshot2', repos='https://packagemanager.posit.co/cran/__linux__/rhel9/latest')"; \
    ...
    Rscript -e "remove.packages('webshot'); install.packages('${PKG}')"; \
    rm -rf "${PKG}"
