...
export interface ExtraTaskConfig {
  name: string;
  imageUrl: string;
  cpu: number;
  memory: number;
...
// Stack props
// ---------------------------------------------------------------------------
export interface MicroarrayStackProps extends cdk.StackProps {
  tier: string;
  appName: string;
  appNamespace: string;
  appService: string;
  ...
  /** Non-web extra task (worker) */
  extraTask: ExtraTaskConfig;
...
// Stack
// ---------------------------------------------------------------------------
export class MicroarrayStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: MicroarrayStackProps) {
    ...
    workerTaskDef.addContainer("WorkerContainer", {
      containerName: et.name,
      image: ecs.ContainerImage.fromRegistry(et.imageUrl),
      essential: true,
      logging: ecs.LogDrivers.awsLogs({
        logGroup: workerLogGroup,
        streamPrefix: et.name,
      }),
    });

    new cdk.CfnOutput(this, "WorkerTaskDefArn", {
      value: workerTaskDef.taskDefinitionArn,
    });

    if (et.createService) {
      const workerService = new ecs.FargateService(this, "WorkerService", {
        serviceName: `${tier}-${appName}-${et.name}`,
        cluster,
      ...
      if (et.nonProdSchedule) {
        const scalable = workerService.autoScaleTaskCount({
          minCapacity: props.scheduledMinCapacity,
          maxCapacity: props.scheduledMaxCapacity,
        });
        scalable.scaleOnSchedule("WorkerScaleOut", {
          schedule: appscaling.Schedule.cron({
            hour: "7",
            minute: "0",
            weekDay: "MON-FRI",
        ...
        scalable.scaleOnSchedule("WorkerScaleIn", {
          schedule: appscaling.Schedule.cron({
            hour: "19",
            minute: "0",
            weekDay: "MON-FRI",
    ...
    const cfnAccessPoint = new efs.CfnAccessPoint(this, "EfsAccessPoint", {
      ...
      accessPointTags: [
        ...
        { key: "Project", value: "analysistools" },
